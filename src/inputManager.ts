...
export class InputManager {
  ...
  input<V = string>(options: InputOptions<V>): Promise<V | null> {
    ...
    return new Promise<V | null>((resolve) => {
      ...
      if (typeof options.timeoutMs === 'number' && options.timeoutMs > 0) {
        const t = setTimeout(() => this.cancelPrompt(id, 'timeout', resolve as (v: unknown) => void), options.timeoutMs);
  ...
  private cancelPrompt(id: string, reason: InputEvents['type'] extends never ? never : 'abort' | 'timeout' | 'missing-renderer' | 'manual', directResolve?: (v: unknown) => void) {
  ...
  private processQueue() {
    if (!this.currentPrompt && this.queue.length > 0) {
      ...
      this.emit({ type: 'prompt:shown', id: this.currentPrompt.id, kind: this.currentPrompt.kind });
      this.notify();
    }
  }
