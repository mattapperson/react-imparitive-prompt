...
import { InputProvider, initInput, input, PromptInputRenderer } from '../src'
  ...
  return (
    ...
    >
      ...
      <div style={{ marginTop: '10px' }}>
        ...
        {queueLength > 0 && (
          <span style={{ marginLeft: '10px', color: '#666' }}>({queueLength} more in queue)</span>
        )}
      </div>
    </div>
  )
    ...
    <InputProvider>
      <div style={{ display: 'flex', height: '100vh' }}>
        ...
        <div style={{ flex: 1, padding: '20px' }}>
          ...
          <button type="button" onClick={handleMultipleInputs}>
      ...
      <div style={{ padding: '20px', maxWidth: '600px' }}>
        ...
        <ul>
          {items.map((item, index) => (
            <li key={`item-${index}-${item.substring(0, 10)}`}>{item}</li>
          ))}
        </ul>
      </div>
    </InputProvider>
