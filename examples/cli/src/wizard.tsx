  ...
  useEffect(() => {
    const runWizard = async () => {
      try {
        ...
        if (confirmed && name && description && type && packageManager !== null && typescript !== null && git !== null && license) {
          setConfig({
            name,
            description,
            type,
    ...
    return (
      <Box flexDirection="column" padding={1}>
        ...
        <Box flexDirection="column" borderStyle="round" borderColor="cyan" padding={1}>
          <Text bold color="cyan">Project: {config.name}</Text>
          <Text dimColor>{config.description}</Text>
          <Newline />
          
          <Text>📁 Type: <Text color="yellow">{config.type}</Text></Text>
          <Text>📦 Package Manager: <Text color="yellow">{config.packageManager}</Text></Text>
          <Text>💻 TypeScript: <Text color={config.typescript ? 'green' : 'gray'}>{config.typescript ? 'Yes' : 'No'}</Text></Text>
          <Text>🔧 Git: <Text color={config.git ? 'green' : 'gray'}>{config.git ? 'Yes' : 'No'}</Text></Text>
          <Text>⚖️  License: <Text color="yellow">{config.license}</Text></Text>
        ...
        <Box flexDirection="column" borderStyle="single" borderColor="gray" padding={1}>
          ...
          <Text color="gray">3. {config.packageManager} {config.packageManager === 'npm' ? 'run ' : ''}dev</Text>
        </Box>
      </Box>
    );
  }
